...
import com.example.springboot.backend.apirest.models.services.IMailService;
...
public class MailRestController {
	...
	{
		Mail mailActual = this.mailService.findById(id);
		...
		if(mailActual == null)
		{
			response.put("mensaje", "Error: no se pudo editar el mail ID: "
		...
		try
		{
			mailActual.setEmail(mail.getEmail());
	...
	public ResponseEntity<?> delete(@PathVariable Long id)
	{
		...
		{
			response.put("mensaje", "Error al eliminar el mail en la base de datos");
			response.put("error", e.getMessage().concat(": ").concat(e.getMostSpecificCause().getMessage()));
			return new ResponseEntity<Map<String, Object>>(response, HttpStatus.INTERNAL_SERVER_ERROR);
		}
